...
class HTMLReportGenerator:
                    ...
                    try:
                        ...
                        for resource in resource_list:
                            resources.append({
                                ...
                                "region": region,
                                "resource_type": label,
                                "name": resource.get("Name") or self._extract_resource_name(resource, label),
                                "resource_id": self._extract_resource_id(resource, label),
    ...
    def _extract_resource_id(self, resource, resource_type):
        """
        Extract the resource ID based on common resource ID keys and resource type.
        Handles various AWS resource types dynamically.
        """
        id_keys = {
            "Security Groups": ["GroupId", "SecurityGroupId"],
            "IAM Roles": ["RoleId", "Name"],
            "VPCs": ["VPCId"],
            "Load Balancers": ["LoadBalancerArn"],
            "EC2 Instances": ["InstanceId"],
            "Elastic IPs": ["AllocationId", "PublicIp"],
            "EBS Volumes": ["VolumeId"],
            "EBS Snapshots": ["SnapshotId"],
            "S3 Buckets": ["BucketArn"],
            "CloudFormation Stacks": ["StackName"],
            "RDS Instances": ["DBInstanceIdentifier"],
            "Lambda Functions": ["FunctionArn"],
            "DynamoDB": ["TableName"],
            "IAM Users": ["UserId"]
        }

        possible_ids = id_keys.get(resource_type, ["ResourceId"])
    ...
    def _extract_resource_name(self, resource, resource_type):
        """
        Extract the name of a resource based on common naming conventions and tags.
        """
        name_keys = {
            ...
            "EC2 Instances": ["InstanceId"],
            "Elastic IPs": ["PublicIp"],
            "S3 Buckets": ["BucketName"],
            "CloudFormation Stacks": ["StackName"],
            "RDS Instances": ["DBClusterIdentifier"],
            "Lambda Functions": ["FunctionName"],
            "DynamoDB": ["TableName"],
            "IAM Users": ["UserName"]
        }

        possible_name_keys = name_keys.get(resource_type, ["ResourceName"])
